...
export default function FleetEditor() {
  ...
  const headers = Object.keys(emptyRow).slice(1, 5)
    ...
    <>
      ...
      <div className="max-w-max max-h-max">
        ...
        <div className="w-full">
          ...
          {newFleet.length != 0 && !isLoading && <Chart<Fleet> headers={headers} rows={newFleet} />}
          {data && !newFleet.length && !isLoading && (
            <Chart<Fleet> headers={headers} rows={data} handleEdit={handleEdit} />
          )}
          {/* Queries result info -------------------------------------------------------*/}
