...
import SupplierSel from './SupplierSelector'
...
export default function FlightsSel() {
  ...
  return (
    <div className="w-full text-xl px-2">
                ...
                {flights && flights.length > 0 && (
                  <ul className="text-base space-y-1 max-h-[644px] overflow-y-auto snap-y">
                    {filteredFlights.map((flight: Flight, index: number) => (
                      ...
                      >
                        <i
                          className={`items-center fas fa-plane col-span-1 place-items-center ${
                            selectedFlights.includes(index)
                              ? 'text-teal-600 dark:text-teal-500'
                        ...
                        <span className="col-span-3 place-items-center">
                          {flight.date}
                        </span>
                        <span className="col-span-2">{flight.std}</span>
                        <span className="col-span-2 place-items-center">
                          {flight.flight}
                        </span>
                        <span className="col-span-2 place-items-center">
                          {flight.to}
      ...
      {(!contracts || contracts.length === 0) &&
        flights &&
        flights.length > 0 && <SupplierSel />}
      {(!flights || !flights.length) && <div>No flights</div>}
    </div>
  )
}
