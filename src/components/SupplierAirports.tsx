...
export default function SupplierAirports() {
  ...
  const { data: places, isLoading } = useGetCompanyDataQuery({ type: "places", id: company.id })
  ...
  async function addAirport(place: Airport) {
    setSearch("")
    await insertPlace({ type: "places", values: [{ airport_id: place.id, company_id: company.id }] }).unwrap()
    setAddon(false)
  }
